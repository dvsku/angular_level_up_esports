...
export class ProductCatalogCategoryService {
  ...
  addNewProductCatalogCategory(productCatalogCategory : ProductCatalogCategory) : Observable<boolean>{
    ...
    return this.httpClient.post<boolean>(url , productCatalogCategory).pipe(tap(data =>{
      console.log("Added new product catalog category -> " + data);
    }));
  }

  editExistingProductCatalogCategory(id : number ,productCatalogCategory : ProductCatalogCategory ) : Observable<boolean>{
    const url = `${this.catalogCategoryAdminurl}/${id}/edit`;
    return this.httpClient.put<boolean>(url , productCatalogCategory).pipe(tap(data =>{
